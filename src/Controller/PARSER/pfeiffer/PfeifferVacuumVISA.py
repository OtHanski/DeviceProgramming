...
### Converted from pyserial to pyvisa by Otto Hanski

import pyvisa as visa

class MaxiGauge (object):
    def __init__(self, settings, baud=9600, debug=False):
        self.rm = visa.ResourceManager()
        self.debug=debug
        testID = settings["MAXIGAUGE"]["ADDRESS"]
        found_meter = False
        try:
            self.instr = self.rm.open_resource(testID)
            if self.identify():
                found_meter = True
        except:
            print("Maxigauge pressuremeter: wrong VISA address in settings, searching for device")
            for ID in self.rm.list_resources():
                try:
                    self.instr = self.rm.open_resource(ID)
                    if self.identify():
                        found_meter = True
                        break
                    else:
                    ...
                    print(repr(ex))
                    print("Failed at "+repr(ID)+", trying next VISA ID")
                    
        
        if not found_meter:
            print("MAXIGAUGE PRESSUREMETER NOT FOUND ON SYSTEM, PLEASE RESTART")
    ...
    def identify(self, CheckID = "TPG366,PTG28770"):
        ID = self.send("AYT",1)[0]  
        debug = False
        if debug: print(CheckID.upper()+" comparing to "+ID.upper())
        if CheckID.upper() in ID.upper():
            isMaxigauge = True
        else:
            isMaxigauge = False
        return isMaxigauge
    ...
    def send(self, mnemonic, numEnquiries = 0):
        ...
        # For some reason there is a NAK left in the buffer after reading pressure. If left unread, it will clog up the buffer.
        self.instr.read()
        return response

    def write(self,what):
        self.debugMessage(what)
        self.instr.write(what)
